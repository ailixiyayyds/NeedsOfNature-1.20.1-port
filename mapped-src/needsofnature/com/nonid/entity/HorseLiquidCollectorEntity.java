...
implements GeoEntity {
    ...
    protected void initDataTracker(DataTracker.Builder builder) {
        ...
        builder.add(COLLECTOR_FULL, (Object)false);
    }
    ...
    public void tick() {
        ...
        if (!this.getEntityWorld().isClient() && this.deathTime > 0) {
            this.discard();
            return;
        }
    ...
    public boolean damage(ServerWorld world, DamageSource source, float amount) {
        ...
        if (this.isInvulnerableTo(world, source) || amount <= 0.0f) {
            return false;
        }
        if (this.getHealth() - amount <= 0.0f) {
            ...
            if (!(attacker instanceof PlayerEntity) || !(player = (PlayerEntity)attacker).isCreative()) {
                this.dropStack(world, new ItemStack((ItemConvertible)NeedsOfNature.HORSE_LIQUID_COLLECTOR));
            }
            this.discard();
            return true;
        }
    ...
    public void readData(ReadView storage) {
        ...
        String raw = storage.getOptionalString(LIQUID_TYPE_NBT).orElse("");
        if (raw.isBlank()) {
            this.setStoredEmpty();
            return;
        }
    ...
    private void setStoredEntityLiquid(Identifier entityTypeId) {
        ...
        this.dataTracker.set(COLLECTOR_FULL, (Object)true);
    ...
    private void setStoredEmpty() {
        this.storedLiquidEntityTypeId = null;
        this.storedMixedLiquid = false;
        this.dataTracker.set(COLLECTOR_FULL, (Object)false);
