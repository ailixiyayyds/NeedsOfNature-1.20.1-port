...
implements GeoEntity {
    ...
    public static DefaultAttributeContainer.Builder createAttributes() {
        return MobEntity.createMobAttributes().add(EntityAttributes.MAX_HEALTH, 2.0).add(EntityAttributes.MOVEMENT_SPEED, 0.0);
    }
    ...
    protected void initDataTracker(DataTracker.Builder builder) {
        ...
        builder.add(TARGET_ENTITY_TYPE_ID, (Object)"");
        builder.add(HATCH_START_GLOBAL_TICK, (Object)0L);
        builder.add(HATCH_AT_GLOBAL_TICK, (Object)0L);
        builder.add(EGG_START_SIZE, (Object)Float.valueOf(0.5f));
        builder.add(EGG_END_SIZE, (Object)Float.valueOf(1.0f));
        builder.add(EGG_TEXTURE_ID, (Object)"");
        builder.add(EGG_HEALTH, (Object)Float.valueOf(2.0f));
    }
    ...
    public void configure(@Nullable UUID ownerPlayerUuid, @Nullable Identifier targetEntityTypeId, long hatchAtTick, @Nullable NeedsOfNature.PregnancyVariantData variantData, @Nullable NonConfig.EggProfile eggProfile) {
        ...
        long startTick = PregnancyEggEntity.resolveCurrentTick(this.getEntityWorld());
    ...
    public void setTargetEntityTypeId(@Nullable Identifier entityTypeId) {
        this.dataTracker.set(TARGET_ENTITY_TYPE_ID, (Object)(entityTypeId == null ? "" : entityTypeId.toString()));
    }
    ...
    public void setHatchStartGlobalTick(long tick) {
        this.dataTracker.set(HATCH_START_GLOBAL_TICK, (Object)Math.max(0L, tick));
    }
    ...
    public void setHatchAtGlobalTick(long tick) {
        this.dataTracker.set(HATCH_AT_GLOBAL_TICK, (Object)Math.max(0L, tick));
    }
    ...
    public void setEggStartSize(float size) {
        this.dataTracker.set(EGG_START_SIZE, (Object)Float.valueOf(PregnancyEggEntity.clampEggSize(size)));
        this.refreshEggDimensionsIfNeeded(true);
    }
    ...
    public void setEggEndSize(float size) {
        this.dataTracker.set(EGG_END_SIZE, (Object)Float.valueOf(PregnancyEggEntity.clampEggSize(size)));
        this.refreshEggDimensionsIfNeeded(true);
    }
    ...
    public void setEggTextureId(@Nullable Identifier textureId) {
        this.dataTracker.set(EGG_TEXTURE_ID, (Object)(textureId == null || textureId.getPath().isBlank() ? "" : textureId.toString()));
    }
    ...
    private void setEggHealth(float health, boolean healToMax) {
        ...
        this.dataTracker.set(EGG_HEALTH, (Object)Float.valueOf(clamped));
    ...
    public void tick() {
        ...
        World class_19372 = this.getEntityWorld();
    ...
    public void writeData(WriteView storage) {
        String variantJson;
        super.writeData(storage);
        Identifier target = this.getTargetEntityTypeId();
        if (target != null) {
            storage.putString(NBT_TARGET_ENTITY_TYPE, target.toString());
        }
        storage.put(NBT_HATCH_START_GLOBAL_TICK, (Codec)Codec.LONG, (Object)this.getHatchStartGlobalTick());
        storage.put(NBT_HATCH_GLOBAL_TICK, (Codec)Codec.LONG, (Object)this.getHatchAtGlobalTick());
    ...
    public void readData(ReadView storage) {
        super.readData(storage);
        String targetRaw = storage.getOptionalString(NBT_TARGET_ENTITY_TYPE).orElse("");
        this.setTargetEntityTypeId(Identifier.tryParse((String)targetRaw));
        long start = Math.max(0L, storage.read(NBT_HATCH_START_GLOBAL_TICK, (Codec)Codec.LONG).orElse(0L));
        long end = Math.max(0L, storage.read(NBT_HATCH_GLOBAL_TICK, (Codec)Codec.LONG).orElse(0L));
        if (end > 0L && start >= end) {
            start = Math.max(0L, end - 1L);
        }
        this.setHatchStartGlobalTick(start);
        this.setHatchAtGlobalTick(end);
        String ownerRaw = storage.getOptionalString(NBT_OWNER_UUID).orElse("");
        this.ownerUuid = PregnancyEggEntity.parseUuid(ownerRaw);
        this.variantData = NeedsOfNature.decodePregnancyVariantData(storage.getOptionalString(NBT_VARIANT_DATA).orElse(""));
        this.setEggStartSize(storage.read(NBT_EGG_START_SIZE, (Codec)Codec.FLOAT).orElse(Float.valueOf(0.5f)).floatValue());
        this.setEggEndSize(storage.read(NBT_EGG_END_SIZE, (Codec)Codec.FLOAT).orElse(Float.valueOf(1.0f)).floatValue());
        this.setEggTextureId(Identifier.tryParse((String)storage.getOptionalString(NBT_EGG_TEXTURE_ID).orElse("")));
        this.setEggHealth(storage.read(NBT_EGG_HEALTH, (Codec)Codec.FLOAT).orElse(Float.valueOf(2.0f)).floatValue());
        this.refreshEggDimensionsIfNeeded(true);
    }
    ...
    public float getGrowthScale(float partialTick) {
        ...
        double now = (float)PregnancyEggEntity.resolveCurrentTick(this.getEntityWorld()) + Math.max(0.0f, partialTick);
    ...
    private void applyEggHealth(float health, boolean healToMax) {
        EntityAttributeInstance attribute = this.getAttributeInstance(EntityAttributes.MAX_HEALTH);
    ...
    private void pushAwayFromNearbyEggs() {
        ...
        List nearbyEggs = this.getEntityWorld().getEntitiesByClass(PregnancyEggEntity.class, searchBox, egg -> egg != this && !egg.isRemoved());
        if (nearbyEggs.isEmpty()) {
            return;
        }
        double pushX = 0.0;
        double pushZ = 0.0;
        double ownRadius = Math.max(0.1, this.getBoundingBox().getLengthX() * 0.5);
        for (PregnancyEggEntity other : nearbyEggs) {
            double distance;
            double dz;
            double otherRadius = Math.max(0.1, other.getBoundingBox().getLengthX() * 0.5);
