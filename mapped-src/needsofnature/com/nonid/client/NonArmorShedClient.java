...
public final class NonArmorShedClient {
    ...
    private static void spawnForPlayer(AbstractClientPlayerEntity player, long startTick) {
        ...
        Vec3d forward = new Vec3d((double)(-MathHelper.sin((double)yawRad)), 0.0, (double)MathHelper.cos((double)yawRad));
    ...
    private static void render(WorldRenderContext context) {
        ...
        for (ArmorProp prop : PROPS) {
            ...
            ItemStackEntityRenderState state = new ItemStackEntityRenderState();
            ItemEntity fake = new ItemEntity((World)client.world, prop.position.x, prop.position.y, prop.position.z, prop.stack);
            state.update((Entity)fake, prop.stack, client.getItemModelManager());
            ItemEntityRenderer.renderStack((MatrixStack)matrices, (OrderedRenderCommandQueue)context.commandQueue(), (int)0xF000F0, (ItemStackEntityRenderState)state, (Random)Random.create((long)prop.seed));
            matrices.pop();
        }
    }
