...
final class NonAuraSystem {
    ...
    static void tickPlayerEnergyAura(ServerWorld world, ServerPlayerEntity player) {
        ...
        List mobs = world.getEntitiesByClass(MobEntity.class, searchBox, mob -> mob.isAlive() && !mob.isRemoved() && mob.getEntityWorld() == world && mob.squaredDistanceTo((Entity)player) <= radiusSq);
        if (mobs.isEmpty()) {
            return;
        }
    ...
    private static boolean hasPairAnimationWithPlayer(MobEntity mob, ServerPlayerEntity player) {
        ...
        return NeedsOfNature.findAnimationForActors(player.getEntityWorld(), pair, Set.of(), null, false, null, Set.of(), NonInjectorMatchPolicy::allowsAutomaticNonMatch) != null;
    }
