...
public final class NonGatherSystem {
    ...
    public static void reportAutomaticMobPlayerMountBlock(ServerWorld world, MobEntity mob, ServerPlayerEntity player, @Nullable Identifier animationId, String phase) {
        ...
        Identifier vehicleType = vehicle == null ? null : Registries.ENTITY_TYPE.getId((Object)vehicle.getType());
        ...
        MutableText vehicleName = vehicle == null ? Text.literal((String)"a vehicle") : vehicle.getDisplayName();
    ...
    private static void tickMobPlayer(ServerWorld world, Session session, MobEntity mob) {
        ...
        if (NonGatherSystem.closeEnoughToStart((Entity)mob, (Entity)player)) {
            ...
            SESSIONS_BY_INITIATOR.remove(session.initiatorUuid(), session);
            if (instanceId == null) {
                NeedsOfNature.LOGGER.info("NoN gather start failed at close range: mobType={} player={} anim={}", new Object[]{Registries.ENTITY_TYPE.getId((Object)mob.getType()), player.getName().getString(), startCandidate.animationId()});
            }
            return;
        }
        int repathCooldown = Math.max(0, session.repathCooldown() - 1);
    ...
    private static void tickMobMob(ServerWorld world, Session session, MobEntity initiator) {
        ...
        if (NonGatherSystem.closeEnoughToStart((Entity)initiator, (Entity)partner)) {
            ...
            SESSIONS_BY_INITIATOR.remove(session.initiatorUuid(), session);
            if (instanceId == null) {
                NeedsOfNature.LOGGER.info("NoN mob gather start failed at close range: first={} second={} anim={}", new Object[]{Registries.ENTITY_TYPE.getId((Object)initiator.getType()), Registries.ENTITY_TYPE.getId((Object)partner.getType()), startCandidate.animationId()});
            }
            return;
        }
        int repathCooldown = Math.max(0, session.repathCooldown() - 1);
    ...
    private static void tickJoin(ServerWorld world, Session session, MobEntity joiningMob) {
        ...
        if (NonGatherSystem.closeEnoughToJoin((Entity)joiningMob, (Entity)requester)) {
            ...
            SESSIONS_BY_INITIATOR.remove(session.initiatorUuid(), session);
            if (!accepted) {
                NeedsOfNature.LOGGER.info("NoN join gather failed at close range: mobType={} player={} instance={} anim={}", new Object[]{Registries.ENTITY_TYPE.getId((Object)joiningMob.getType()), requester.getName().getString(), session.activeInstanceUuid(), session.animationId()});
