...
final class NonTrinketsCompat {
    ...
    static void registerPredicates() {
        TrinketsApi.registerTrinketPredicate((Identifier)NeedsOfNature.id("female"), (stack, slot, entity) -> NonAccessoryBehavior.isSlot(slot.getId(), "legs/v") && !NonGenderSystem.hasFemaleGender((Entity)entity) ? TriState.FALSE : TriState.DEFAULT);
        TrinketsApi.registerTrinketPredicate((Identifier)NeedsOfNature.id("male"), (stack, slot, entity) -> NonAccessoryBehavior.isSlot(slot.getId(), "legs/d") && !NonGenderSystem.hasMaleGender((Entity)entity) ? TriState.FALSE : TriState.DEFAULT);
    }
    ...
    static NonAccessoryEffects getGlobalAccessoryEffects(LivingEntity entity) {
        ...
        return TrinketsApi.getTrinketComponent((LivingEntity)entity).map(TrinketComponent::getAllEquipped).orElse(List.of()).stream().filter(pair -> !NonTrinketsCompat.isPendingUnequippedSlot(entity, (SlotReference)pair.getLeft())).filter(pair -> {
            Identifier itemId = Registries.ITEM.getId((Object)((ItemStack)pair.getRight()).getItem());
            return !NonAccessoryShedSystem.isSlotEffectSuppressed(entity.getUuid(), ((SlotReference)pair.getLeft()).getId(), itemId);
        }).map(pair -> Registries.ITEM.getId((Object)((ItemStack)pair.getRight()).getItem())).map(NonAccessoryDefinitions::getEffects).map(NonAccessoryEffects::globalOnly).reduce(NonAccessoryEffects.NEUTRAL, NonAccessoryEffects::combine);
    }
    ...
    static NonAccessoryEffects getActiveTankAccessoryEffects(LivingEntity entity) {
        ...
        return TrinketsApi.getTrinketComponent((LivingEntity)entity).map(TrinketComponent::getAllEquipped).orElse(List.of()).stream().filter(pair -> !NonTrinketsCompat.isPendingUnequippedSlot(entity, (SlotReference)pair.getLeft())).filter(pair -> NonAccessoryBehavior.isSlot(((SlotReference)pair.getLeft()).getId(), activeSlotId)).filter(pair -> {
            Identifier itemId = Registries.ITEM.getId((Object)((ItemStack)pair.getRight()).getItem());
            return !NonAccessoryShedSystem.isSlotEffectSuppressed(entity.getUuid(), ((SlotReference)pair.getLeft()).getId(), itemId);
        }).map(pair -> Registries.ITEM.getId((Object)((ItemStack)pair.getRight()).getItem())).map(NonAccessoryDefinitions::getEffects).map(NonAccessoryEffects::tankScopedOnly).reduce(NonAccessoryEffects.NEUTRAL, NonAccessoryEffects::combine);
    }
    ...
    static boolean blocksPregnancy(ServerPlayerEntity player) {
        ...
        return TrinketsApi.getTrinketComponent((LivingEntity)player).map(TrinketComponent::getAllEquipped).orElse(List.of()).stream().filter(pair -> !NonTrinketsCompat.isPendingUnequippedSlot((LivingEntity)player, (SlotReference)pair.getLeft())).anyMatch(pair -> {
            ...
            String slotId = ((SlotReference)pair.getLeft()).getId();
    ...
    static boolean hasProtectorFor(ServerPlayerEntity player, NonLiquidRoles.InjectorRole role) {
        ...
        return TrinketsApi.getTrinketComponent((LivingEntity)player).map(TrinketComponent::getAllEquipped).orElse(List.of()).stream().filter(pair -> !NonTrinketsCompat.isPendingUnequippedSlot((LivingEntity)player, (SlotReference)pair.getLeft())).anyMatch(pair -> {
            ...
            return NonAccessoryDefinitions.getBehavior(Registries.ITEM.getId((Object)stack.getItem())).blocks(role);
        });
    }
    ...
    static boolean consumeProtectorFor(ServerWorld world, ServerPlayerEntity player, NonLiquidRoles.InjectorRole role) {
        ...
        for (Pair pair : component.getAllEquipped()) {
            ...
            ItemStack stack;
            if (NonTrinketsCompat.isPendingUnequippedSlot((LivingEntity)player, (SlotReference)pair.getLeft()) || (stack = (ItemStack)pair.getRight()) == null || stack.isEmpty() || !(behavior = NonAccessoryDefinitions.getBehavior(Registries.ITEM.getId((Object)stack.getItem()))).blocks(role)) continue;
            if (Math.max(stack.getMaxDamage(), behavior.maxDurability()) <= 0) {
                return false;
            }
            int cost = Math.max(1, behavior.protectionDurabilityCost());
            NonTrinketsCompat.applyDurabilityCost(world, player, (SlotReference)pair.getLeft(), stack, cost);
            world.playSound(null, player.getX(), player.getY(), player.getZ(), (RegistryEntry)SoundEvents.ITEM_SHIELD_BLOCK, SoundCategory.PLAYERS, 0.75f, 0.9f + world.getRandom().nextFloat() * 0.2f);
    ...
    static void applyInjectionDurabilityCost(ServerWorld world, ServerPlayerEntity player, NonLiquidRoles.InjectorRole role) {
        ...
        for (Pair pair : component.getAllEquipped()) {
            int cost;
            ItemStack stack;
            if (NonTrinketsCompat.isPendingUnequippedSlot((LivingEntity)player, (SlotReference)pair.getLeft()) || (stack = (ItemStack)pair.getRight()) == null || stack.isEmpty()) continue;
            String slotId = ((SlotReference)pair.getLeft()).getId();
    ...
    private static boolean applyDurabilityCost(ServerWorld world, ServerPlayerEntity player, SlotReference slot, ItemStack stack, int cost) {
        ...
        if (stack.getMaxDamage() <= 0) {
            stack.set(DataComponentTypes.MAX_DAMAGE, (Object)maxDamage);
        }
        if (stack.get(DataComponentTypes.DAMAGE) == null) {
            stack.set(DataComponentTypes.DAMAGE, (Object)0);
        }
    ...
    static List<Identifier> resolveEquippedSkinOverlays(LivingEntity entity) {
        ...
        return TrinketsApi.getTrinketComponent((LivingEntity)entity).map(component -> {
            ...
            for (Pair pair : component.getAllEquipped()) {
                ...
                Identifier itemId = Registries.ITEM.getId((Object)((ItemStack)pair.getRight()).getItem());
                if (NonAccessoryShedSystem.isSlotVisuallyShed(entity.getUuid(), ((SlotReference)pair.getLeft()).getId(), itemId) || (overlay = NonAccessoryItemRegistry.getSkinOverlays(itemId).forSlot(((SlotReference)pair.getLeft()).getId())) == null) continue;
    ...
    static List<ItemStack> getVisuallyShedAccessoryStacks(LivingEntity entity) {
        ...
        return TrinketsApi.getTrinketComponent((LivingEntity)entity).map(component -> {
            ...
            for (Pair pair : component.getAllEquipped()) {
                if (pair.getRight() == null || ((ItemStack)pair.getRight()).isEmpty()) continue;
                Identifier itemId = Registries.ITEM.getId((Object)((ItemStack)pair.getRight()).getItem());
                if (!NonAccessoryShedSystem.isSlotVisuallyShed(entity.getUuid(), ((SlotReference)pair.getLeft()).getId(), itemId)) continue;
    ...
    static NonTrinketsIntegration.TankScopeStatus resolveTankScopeStatus(LivingEntity entity, ItemStack stack) {
        ...
        return TrinketsApi.getTrinketComponent((LivingEntity)entity).map(component -> {
            for (Pair pair : component.getAllEquipped()) {
                ...
                return NonTrinketsIntegration.isActiveTankSlot(entity, ((SlotReference)pair.getLeft()).getId()) ? NonTrinketsIntegration.TankScopeStatus.ACTIVE_EQUIPPED : NonTrinketsIntegration.TankScopeStatus.INACTIVE_EQUIPPED;
            }
            return NonTrinketsIntegration.TankScopeStatus.NOT_EQUIPPED;
