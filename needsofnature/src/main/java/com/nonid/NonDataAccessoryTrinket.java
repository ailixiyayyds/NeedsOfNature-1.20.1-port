...
implements Trinket {
    ...
    public boolean canEquip(ItemStack stack, SlotReference slot, LivingEntity entity) {
        ...
        String targetSlot = NonTrinketsCompat.slotId(slot);
    ...
    private static boolean canEquipWithCurrent(TrinketComponent component, ItemStack stack, SlotReference target, NonAccessoryBehavior behavior, String targetSlot) {
        ...
        for (Pair<SlotReference, ItemStack> pair : component.getAllEquipped()) {
            SlotReference otherSlot = pair.getLeft();
            ItemStack otherStack = pair.getRight();
            if (otherSlot == null || otherStack == null || otherStack.isEmpty() || NonDataAccessoryTrinket.isSameSlot(otherSlot, target)) continue;
            NonAccessoryBehavior otherBehavior = NonAccessoryDefinitions.getBehavior(Registries.ITEM.getId(otherStack.getItem()));
            if (otherBehavior.occupiesSlot(targetSlot)) {
                return false;
            }
            for (String occupied : behavior.occupiesSlots()) {
                if (!NonAccessoryBehavior.isSlot(NonTrinketsCompat.slotId(otherSlot), occupied)) continue;
                return false;
            }
    ...
    private static boolean isSameSlot(SlotReference first, SlotReference second) {
        return first != null && second != null && first.index() == second.index() && Objects.equals(NonTrinketsCompat.slotId(first), NonTrinketsCompat.slotId(second));
    }
}
