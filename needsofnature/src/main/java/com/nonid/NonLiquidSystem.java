...
final class NonLiquidSystem {
    ...
    static void syncLiquidState(ServerPlayerEntity player) {
        ...
        NonServerNetworking.send(player, new LiquidStateS2CPayload(stored, capacity, tint));
    ...
    static void updateFilledEffect(ServerPlayerEntity player, LiquidHolder holder) {
        ...
        StatusEffectInstance current = player.getStatusEffect(NonStatusEffects.FILLED);
        if (amplifier < 0) {
            player.removeStatusEffect(NonStatusEffects.FILLED);
            return;
        }
        if (current != null) {
        ...
        player.addStatusEffect(new StatusEffectInstance(NonStatusEffects.FILLED, Integer.MAX_VALUE, amplifier, false, false, true));
    }
