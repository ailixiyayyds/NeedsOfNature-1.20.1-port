...
final class NonGenderSystem {
    ...
    static void registerPlayerGenderRequests() {
        NonServerNetworking.register(SetPlayerGenderC2SPayload.ID, SetPlayerGenderC2SPayload::read, (server, player, payload) -> {
            if (player == null) {
                return;
            }
    ...
    private static void clearPlayerGenderSelectionState(ServerPlayerEntity player) {
        ...
        player.removeScoreboardTag(GENDER_TAG_SELECTED);
        player.removeScoreboardTag(GENDER_TAG_MALE);
        player.removeScoreboardTag(GENDER_TAG_FEMALE);
        if (player instanceof GenderHolder) {
            GenderHolder holder = (GenderHolder)player;
    ...
    private static void sendGenderSelectionPrompt(ServerPlayerEntity player) {
        ...
        NonConfig config = NeedsOfNature.getConfig();
        NonServerNetworking.send(player, new GenderSelectionPromptS2CPayload(config.getAllowedStartingGenderMask(), NonGenderSystem.currentOrDefaultPlayerMask(player), !config.allowPlayerGenderChangeAnytime()));
    }
    ...
    static void applyGenderMask(Entity entity, int mask, NonChangeSource source) {
        ...
        entity.removeScoreboardTag(GENDER_TAG_MALE);
        entity.removeScoreboardTag(GENDER_TAG_FEMALE);
