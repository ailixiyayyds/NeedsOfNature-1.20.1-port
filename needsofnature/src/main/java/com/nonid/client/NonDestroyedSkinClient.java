...
public final class NonDestroyedSkinClient {
    ...
    public static boolean uploadLocalSkinIfPresent() {
        ...
        try {
            ...
            if (ClientPlayNetworking.canSend(DestroyedSkinUploadC2SPayload.ID)) {
                NonClientNetworking.send(new DestroyedSkinUploadC2SPayload(bytes));
                return true;
            }
        }
    ...
    public static boolean hasCurrentDefaultDestroyedSkin() {
        ...
        Identifier baseSkin = client.player.getSkinTexture();
        if (baseSkin == null) {
            return false;
        }
    ...
    private static boolean uploadGeneratedSkinIfPossible() {
        ...
        if (defaultSkinKey != null && (defaultDestroyedSkin = NonDestroyedSkinClient.readDefaultDestroyedSkin(client, defaultSkinKey)) != null && NonDestroyedSkinClient.isValidSkinPng(defaultDestroyedSkin)) {
            NonClientNetworking.send(new DestroyedSkinUploadC2SPayload(defaultDestroyedSkin));
    ...
    public static boolean uploadLocalMasksIfPresent() {
        ...
        for (int stage = 1; stage < 4; ++stage) {
            ...
            try {
                ...
                NonClientNetworking.send(new DestroyedSkinMaskUploadC2SPayload(stage, bytes));
                uploadedAny = true;
                continue;
            }
            catch (IOException | RuntimeException e) {
    ...
    public static void spawnRippedFabricParticles(UUID playerUuid, double x, double y, double z, int count, long seed) {
        ...
        Identifier baseSkin = player.getSkinTexture();
    ...
    private static Identifier getOrCreateTexture(LivingEntity entity, int stage, @Nullable Identifier currentBaseTexture, boolean overlayFallback) {
        ...
        List<Identifier> accessoryOverlays;
        if (entity == null) {
            return null;
        }
        ...
        accessoryOverlays = overlayFallback ? List.of() : NonAccessorySkinOverlays.resolveEquippedOverlays(entity);
        ...
        try {
            ...
            client.getTextureManager().registerTexture(id, (AbstractTexture)new NativeImageBackedTexture(image));
            ...
            if (mask != null) {
                ...
                for (int y = 0; y < height; ++y) {
                    for (int x = 0; x < width; ++x) {
                        ...
                        int destroyedArgb = destroyed.getColor(x, y);
                        out.setColor(x, y, NonDestroyedSkinClient.blendArgb(baseArgb, destroyedArgb, factor));
                    }
                }
                mask.close();
            }
    ...
    public static int sendDetectedRippedSkinInfo() {
        ...
        String model = client.player.getModel();
    ...
    private static DefaultSkinMatch detectDefaultSkinMatchByPixels(MinecraftClient client, Identifier baseSkin) {
        ...
        String currentModel = client.player != null ? client.player.getModel() : "slim";
        String preferredModel = "default".equals(currentModel) ? "wide" : "slim";
    ...
    private static DefaultSkinMatch detectDefaultSkinMatchByPixels(MinecraftClient client, NativeImage base, String model) {
        ...
        for (String name : VANILLA_DEFAULT_SKIN_NAMES) {
            Identifier vanillaId = new Identifier("minecraft", DEFAULT_PLAYER_SKIN_PREFIX + model + "/" + name + ".png");
    ...
    private static double imageMatchConfidence(NativeImage first, NativeImage second) {
        ...
        for (int y = 0; y < first.getHeight(); ++y) {
            for (int x = 0; x < first.getWidth(); ++x) {
                int firstArgb = first.getColor(x, y);
                int secondArgb = second.getColor(x, y);
    ...
    private static NativeImage createMaskedOverlay(MinecraftClient client, UUID playerUuid, byte[] skin, int stage) throws IOException {
        ...
        for (int y = 0; y < destroyed.getHeight(); ++y) {
            for (int x = 0; x < destroyed.getWidth(); ++x) {
                int skinArgb = destroyed.getColor(x, y);
                int maskArgb = mask.getColor(x % maskWidth, y % maskHeight);
                int factor = NonDestroyedSkinClient.maskFactor(maskArgb);
                int alpha = (skinArgb >>> 24 & 0xFF) * factor / 255;
                out.setColor(x, y, skinArgb & 0xFFFFFF | alpha << 24);
            }
        }
        destroyed.close();
        mask.close();
    ...
    private static byte[] createGeneratedDestroyedSkin(MinecraftClient client, Identifier baseSkin) throws IOException {
        ...
        boolean wide = client != null && client.player != null && "default".equals(client.player.getModel());
        try (base; NativeImage destroyedBase = NonDestroyedSkinClient.readDestroyedBaseTexture(client, wide); NativeImage out = new NativeImage(NativeImage.Format.RGBA, base.getWidth(), base.getHeight(), false)) {
            if (base.getWidth() != 64 || base.getHeight() != 64) {
                return null;
            }
            int detectedTint = NonDestroyedSkinClient.detectSkinTint(base);
            NonConfig config = NeedsOfNature.getConfig();
            int tintRgb = config.getDestroyedSkinTintRgb(detectedTint);
            if (!config.hasDestroyedSkinTintOverride()) {
                config.setDestroyedSkinTintHex(NonDestroyedSkinClient.formatRgb(tintRgb));
                config.save();
            }
            int width = Math.min(base.getWidth(), destroyedBase.getWidth());
            int height = Math.min(base.getHeight(), destroyedBase.getHeight());
            for (int y = 0; y < height; ++y) {
                for (int x = 0; x < width; ++x) {
                    int pixel = destroyedBase.getColor(x, y);
    ...
    private static int detectSkinTint(NativeImage skin) {
        ...
        ArrayList<ColorBucket> topBuckets = new ArrayList<ColorBucket>(buckets.values());
    ...
    private static NativeImage readMask(MinecraftClient client, UUID playerUuid, int stage) {
        ...
        block14: {
            ...
            try (InputStream in = resource.getInputStream()) {
                LootTableData = NativeImage.read(in);
            }
            catch (IOException | RuntimeException e) {
    ...
    private static void applyRawOverlay(NativeImage out, NativeImage overlay) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                int overlayArgb = overlay.getColor(x, y);
                int alpha = overlayArgb >>> 24 & 0xFF;
                if (alpha <= 0) continue;
                out.setColor(x, y, NonDestroyedSkinClient.blendArgb(out.getColor(x, y), overlayArgb, alpha));
            }
        }
    }
    ...
    private static void applyTintMask(NativeImage out, NativeImage mask, int tintRgb, int alpha) {
        ...
        for (int y = 0; y < out.getHeight(); ++y) {
            for (int x = 0; x < out.getWidth(); ++x) {
                int factor = NonDestroyedSkinClient.maskFactor(mask.getColor(x % maskWidth, y % maskHeight)) * alpha / 255;
                if (factor <= 0) continue;
                out.setColor(x, y, NonDestroyedSkinClient.blendArgb(out.getColor(x, y), overlayBase, factor));
            }
        }
    }
