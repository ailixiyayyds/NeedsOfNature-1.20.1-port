...
public final class NonIntifaceClient {
    ...
    public static Text statusText(NonConfig config) {
        ...
        return Text.translatable((String)("config.needsofnature.intiface.status." + state.name().toLowerCase(Locale.ROOT)), (Object[])new Object[]{devices}).append((Text)Text.literal((String)suffix));
    }
    ...
    private static void connectInternal(String url) {
        ...
        try {
            ...
            nextClient.setDeviceAddedHandler(device -> {
                ...
                state = State.CONNECTED;
                detail = Text.translatable((String)"config.needsofnature.intiface.status.device_added", (Object[])new Object[]{device.getDisplayName()}).getString();
            ...
            nextClient.setDeviceRemovedHandler(index -> {
                ...
                state = State.CONNECTED;
                detail = Text.translatable((String)"config.needsofnature.intiface.status.device_removed").getString();
            });
            nextClient.setScanningFinishedHandler(() -> {
                ...
                state = State.CONNECTED;
                detail = Text.translatable((String)"config.needsofnature.intiface.status.scan_finished").getString();
            });
    ...
    private static boolean sendVibrate(ButtplugClientWSClient activeClient, double scalar) {
        List<ButtplugClientDevice> devices = activeClient.getDevices();
        if (devices.isEmpty()) {
            detail = Text.translatable((String)"config.needsofnature.intiface.status.no_devices").getString();
            return false;
        }
        boolean sentAny = false;
        ...
        if (!sentAny) {
            detail = Text.translatable((String)"config.needsofnature.intiface.status.no_vibrate_devices").getString();
        }
        return sentAny;
    }
    ...
    private static void stopAllInternal(boolean userVisible) {
        ...
        try {
            ...
            currentOscillatorScalar = 0.0;
            if (userVisible) {
                detail = Text.translatable((String)"config.needsofnature.intiface.status.stopped").getString();
            }
        }
        catch (Exception e) {
