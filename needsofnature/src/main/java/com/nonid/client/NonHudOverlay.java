...
public final class NonHudOverlay {
    private static final Identifier ENERGY_BAR_BG = NeedsOfNature.id("textures/gui/sprites/hud/progress_background.png");
    private static final Identifier ENERGY_BAR_FILL = NeedsOfNature.id("textures/gui/sprites/hud/progress_full.png");
    private static final Identifier ATTACK_BAR_BG = NeedsOfNature.id("textures/gui/sprites/hud/attack_background.png");
    private static final Identifier ATTACK_BAR_FILL = NeedsOfNature.id("textures/gui/sprites/hud/attack_full.png");
    private static final Identifier ATTACK_PROMPT = NeedsOfNature.id("textures/gui/sprites/hud/ad_mash.png");
    private static final Identifier LIQUID_TANK_BG = NeedsOfNature.id("textures/gui/sprites/hud/liquid_background.png");
    private static final Identifier LIQUID_TANK_FILL = NeedsOfNature.id("textures/gui/sprites/hud/liquid_full.png");
    ...
    public static void renderOverlay(DrawContext ctx) {
        ...
        if (visibility.energy() && progress >= 0.0f) {
            ...
            drawTexture(ctx, ENERGY_BAR_BG, energyX, energyY, texW, texH);
            if (filled > 0) {
                ctx.enableScissor(energyX, energyY, energyX + filled, energyY + texH);
                drawTexture(ctx, ENERGY_BAR_FILL, energyX, energyY, texW, texH);
                ctx.disableScissor();
            }
        }
        if (client.player != null && attackInstance != null && AfwClientAnimationRuntime.isActorPendingOrActive((UUID)client.player.getUuid()) && ((inst = AfwClientAnimationRuntime.findLatestActiveInstanceContaining((UUID)client.player.getUuid())) == null || inst.equals(attackInstance)) && attackEscapable) {
            ...
            if (visibility.attack()) {
                ...
                drawTexture(ctx, ATTACK_BAR_BG, attackBarX, attackBarY, attackBarW, attackBarH);
                if (attackFilled > 0) {
                    ctx.enableScissor(attackBarX, attackBarY, attackBarX + attackFilled, attackBarY + attackBarH);
                    drawTexture(ctx, ATTACK_BAR_FILL, attackBarX, attackBarY, attackBarW, attackBarH);
                    ctx.disableScissor();
                }
            }
            if (visibility.prompt()) {
                ...
                ctx.drawTexture(ATTACK_PROMPT, drawX, drawY, 0.0f, (float)v, frameW, frameH, promptInfo.textureWidth(), promptInfo.textureHeight());
                matrices.pop();
            }
        }
    ...
    private static void renderLiquidOverlayInternal(DrawContext ctx, boolean preview) {
        ...
        drawTexture(ctx, LIQUID_TANK_BG, x, y, scaledW, scaledH);
        if (filled > 0) {
            ...
            drawTintedTexture(ctx, LIQUID_TANK_FILL, x, y, scaledW, scaledH, fillColor);
            ctx.disableScissor();
        }
    }
    ...
    private static void renderEnergyHeart(DrawContext ctx, int x, int y, boolean preview) {
        ...
        matrices.scale(scale, scale, 1.0f);
        matrices.translate(-centerX, -centerY, 0.0);
        drawTintedTexture(ctx, ENERGY_HEART, x, y, 8, 8, color);
        matrices.pop();
    }
    ...
    public static int getEnergizedLevel(ClientPlayerEntity player) {
        ...
        for (StatusEffectInstance effect : player.getStatusEffects()) {
            if (effect.getEffectType() != NonStatusEffects.ENERGIZED) continue;
    ...
    private static void drawBar(DrawContext ctx, Identifier bg, Identifier fill, int x, int y, int w, int h, float progress) {
        ...
        drawTexture(ctx, bg, x, y, w, h);
        if (filled > 0) {
            ctx.enableScissor(x, y, x + filled, y + h);
            drawTexture(ctx, fill, x, y, w, h);
            ctx.disableScissor();
        }
    }
    ...
    private static AttackPromptInfo resolveAttackPromptInfo() {
        return new AttackPromptInfo(68, 128);
    }

    private static SpriteSize resolveSpriteSize(Identifier id, int fallbackW, int fallbackH) {
        if (ATTACK_BAR_BG.equals(id) || ATTACK_BAR_FILL.equals(id)) {
            return new SpriteSize(81, 5);
        }
        if (LIQUID_TANK_BG.equals(id) || LIQUID_TANK_FILL.equals(id)) {
            return new SpriteSize(8, 31);
        }
        if (ENERGY_BAR_BG.equals(id) || ENERGY_BAR_FILL.equals(id)) {
            return new SpriteSize(182, 5);
        }
    ...
    private static void drawTexture(DrawContext context, Identifier texture, int x, int y, int width, int height) {
        context.drawTexture(texture, x, y, 0.0f, 0.0f, width, height, width, height);
    }

    private static void drawTintedTexture(DrawContext context, Identifier texture, int x, int y, int width, int height, int argb) {
