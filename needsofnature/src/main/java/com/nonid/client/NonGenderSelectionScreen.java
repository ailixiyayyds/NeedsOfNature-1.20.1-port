...
extends Screen {
    ...
    private void choose(NonConfig.PlayerGenderSelection selection) {
        ...
        if (ClientPlayNetworking.canSend(SetPlayerGenderC2SPayload.ID)) {
            NonClientNetworking.send(new SetPlayerGenderC2SPayload(selection.mask(), true));
        }
    ...
    private Text genderText(NonConfig.PlayerGenderSelection selection) {
        MutableText base = switch (selection) {
            case MALE -> Text.translatable("screen.needsofnature.gender_selection.option.male").copy().formatted(Formatting.AQUA);
            case FEMALE -> Text.translatable("screen.needsofnature.gender_selection.option.female").copy().formatted(Formatting.LIGHT_PURPLE);
            case BOTH -> Text.translatable("screen.needsofnature.gender_selection.option.both").copy().formatted(Formatting.GOLD);
        };
