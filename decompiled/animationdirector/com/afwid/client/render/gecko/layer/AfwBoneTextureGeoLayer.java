...
extends GeoRenderLayer<T, O, R> {
    ...
    public void submitRenderTask(RenderPassInfo<@NotNull R> renderPassInfo, @NotNull class_11659 renderTasks) {
        ...
        Map<String, class_2960> boneTextures = this.getBoneTextures(renderPassInfo.renderState());
        if (boneTextures.isEmpty()) {
            return;
        }
        LinkedHashMap<class_2960, Set> bonesByTexture = new LinkedHashMap<class_2960, Set>();
        for (Map.Entry<String, class_2960> entry : boneTextures.entrySet()) {
        ...
        for (Map.Entry entry : bonesByTexture.entrySet()) {
            ...
            class_2960 texture = (class_2960)entry.getKey();
            ...
            renderTasks.method_73483(renderPassInfo.poseStack(), renderLayer, (entryMatrices, vertexConsumer) -> renderPassInfo.renderPosed(() -> {
                class_4587 modelStack = new class_4587();
                modelStack.method_23760().method_66521(entryMatrices);
    ...
    private Map<String, class_2960> getBoneTextures(R renderState) {
        Map boneTextures = (Map)renderState.getOrDefaultGeckolibData(AfwGeckoTickets.BONE_TEXTURES, Map.of());
        return boneTextures;
    }

    private void renderBoneFiltered(class_4587 poseStack, GeoBone bone, class_4588 buffer, Set<String> allowedBones, Set<String> explicitlyHiddenBones, int packedOverlay, int packedLight, int renderColor) {
    ...
    private void renderCube(GeoCube cube, class_4587 poseStack, class_4588 buffer, int packedOverlay, int packedLight, int renderColor) {
