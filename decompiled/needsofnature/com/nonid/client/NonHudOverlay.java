...
public final class NonHudOverlay {
    private static final class_2960 ENERGY_BAR_BG = class_2960.method_60655((String)"needsofnature", (String)"hud/progress_background");
    private static final class_2960 ENERGY_BAR_FILL = class_2960.method_60655((String)"needsofnature", (String)"hud/progress_full");
    private static final class_2960 ATTACK_BAR_BG = class_2960.method_60655((String)"needsofnature", (String)"hud/attack_background");
    private static final class_2960 ATTACK_BAR_FILL = class_2960.method_60655((String)"needsofnature", (String)"hud/attack_full");
    private static final class_2960 ATTACK_PROMPT = class_2960.method_60655((String)"needsofnature", (String)"hud/ad_mash");
    private static final class_2960 LIQUID_TANK_BG = class_2960.method_60655((String)"needsofnature", (String)"hud/liquid_background");
    private static final class_2960 LIQUID_TANK_FILL = class_2960.method_60655((String)"needsofnature", (String)"hud/liquid_full");
    private static final class_2960 ENERGY_HEART = class_2960.method_60655((String)"needsofnature", (String)"textures/particle/smallheart.png");
    ...
    public static void clientTick(class_310 client) {
        ...
        boolean animActive = AfwClientAnimationRuntime.isActorPendingOrActive((UUID)player.method_5667());
        ...
        if (lastUpdateTick != (now = client.field_1687.method_75260())) {
            lastUpdateTick = now;
            loopProgress = NonHudOverlay.computeLoopProgress(player.method_5667(), now);
            NonHudOverlay.tickAttackDecay(now);
        }
    }

    public static void onDisconnect(class_310 client) {
        NonHudOverlay.clearAnimationHudState();
        liquidStored = 0;
        liquidTintRgb = 15920063;
    ...
    public static boolean shouldRenderGameplayHud() {
        ...
        return client != null && client.field_1724 != null && client.field_1690 != null && !client.field_1690.field_1842 && !client.field_1724.method_7325();
    }
    ...
    public static void renderOverlay(class_332 ctx) {
        ...
        int width = ctx.method_51421();
        int height = ctx.method_51443();
        int texW = 182;
        int texH = 5;
        ...
        if (client.field_1724 != null && client.field_1687 != null) {
            ...
            displayedLoopProgress = NonHudOverlay.computeLoopProgress(client.field_1724.method_5667(), (float)client.field_1687.method_75260() + Math.max(0.0f, tickProgress));
        }
        ...
        if (visibility.energy() && progress >= 0.0f) {
            ...
            ctx.method_52706(class_10799.field_56883, ENERGY_BAR_BG, energyX, energyY, texW, texH);
            if (filled > 0) {
                ctx.method_44379(energyX, energyY, energyX + filled, energyY + texH);
                ctx.method_52706(class_10799.field_56883, ENERGY_BAR_FILL, energyX, energyY, texW, texH);
                ctx.method_44380();
            }
        }
        if (client.field_1724 != null && attackInstance != null && AfwClientAnimationRuntime.isActorPendingOrActive((UUID)client.field_1724.method_5667()) && ((inst = AfwClientAnimationRuntime.findLatestActiveInstanceContaining((UUID)client.field_1724.method_5667())) == null || inst.equals(attackInstance)) && attackEscapable) {
            ...
            if (visibility.attack()) {
                ...
                ctx.method_52706(class_10799.field_56883, ATTACK_BAR_BG, attackBarX, attackBarY, attackBarW, attackBarH);
                if (attackFilled > 0) {
                    ctx.method_44379(attackBarX, attackBarY, attackBarX + attackFilled, attackBarY + attackBarH);
                    ctx.method_52706(class_10799.field_56883, ATTACK_BAR_FILL, attackBarX, attackBarY, attackBarW, attackBarH);
                    ctx.method_44380();
                }
            }
            if (visibility.prompt()) {
                ...
                ctx.method_70846(class_10799.field_56883, ATTACK_PROMPT, promptInfo.textureWidth(), promptInfo.textureHeight(), 0, v, drawX, drawY, frameW, frameH);
                matrices.popMatrix();
            }
        }
    ...
    public static void renderLiquidOverlay(class_332 ctx) {
        if (!NonHudOverlay.shouldRenderGameplayHud()) {
            return;
        }
    ...
    public static void renderUiPreview(class_332 ctx) {
        class_310 client = class_310.method_1551();
        if (client == null || ctx == null) {
            return;
        }
    ...
    public static UiPreviewBounds getUiPreviewBounds(class_332 ctx) {
        if (ctx == null) {
    ...
    private static void renderLiquidOverlayInternal(class_332 ctx, boolean preview) {
        class_310 client = class_310.method_1551();
        if (client == null || ctx == null) {
            return;
        }
        ...
        ctx.method_52706(class_10799.field_56883, LIQUID_TANK_BG, x, y, scaledW, scaledH);
        if (filled > 0) {
            int fillY = y + scaledH - filled;
            ctx.method_44379(x, fillY, x + scaledW, y + scaledH);
            int fillColor = preview ? -1 : NonLiquidColors.toOpaqueArgb(liquidTintRgb);
            ctx.method_52707(class_10799.field_56883, LIQUID_TANK_FILL, x, y, scaledW, scaledH, fillColor);
            ctx.method_44380();
        }
    }

    private static void renderEnergyHeartOverlay(class_332 ctx, boolean preview) {
    ...
    private static Rect energyHeartRect(class_332 ctx, boolean preview) {
        int width = ctx.method_51421();
        int height = ctx.method_51443();
        SpriteSize bgSize = NonHudOverlay.resolveSpriteSize(LIQUID_TANK_BG, 16, 32);
        int tankW = bgSize.width();
    ...
    private static void renderEnergyHeart(class_332 ctx, int x, int y, boolean preview) {
        int energizedLevel;
        class_310 client = class_310.method_1551();
        if (client == null || ctx == null) {
            return;
        }
        int n = energizedLevel = preview ? 2 : NonHudOverlay.getEnergizedLevel(client.field_1724);
        if (energizedLevel <= 0) {
            return;
        }
        ...
        matrices.scale(scale, scale);
        matrices.translate((float)(-centerX), (float)(-centerY));
        ctx.method_25291(class_10799.field_56883, ENERGY_HEART, x, y, 0.0f, 0.0f, 8, 8, 8, 8, color);
        matrices.popMatrix();
    }

    public static int getEnergizedLevel(class_746 player) {
        if (player == null) {
            return 0;
        }
        for (class_1293 effect : player.method_6026()) {
            if (effect.method_5579().comp_349() != NonStatusEffects.ENERGIZED) continue;
            return Math.max(1, Math.min(3, effect.method_5578() + 1));
        }
        return 0;
    }
    ...
    private static void drawBar(class_332 ctx, class_2960 bg, class_2960 fill, int x, int y, int w, int h, float progress) {
        float clamped = Math.max(0.0f, Math.min(1.0f, progress));
        int filled = Math.max(0, Math.min(w, Math.round(clamped * (float)w)));
        ctx.method_52706(class_10799.field_56883, bg, x, y, w, h);
        if (filled > 0) {
            ctx.method_44379(x, y, x + filled, y + h);
            ctx.method_52706(class_10799.field_56883, fill, x, y, w, h);
            ctx.method_44380();
        }
    }
    ...
    private static double resolveFallbackLoopDurationTicks(AnimationStageInfo currentStage, double speed) {
        ...
        class_2960 stageAnimationId = currentStage.animationId();
    ...
    private static AttackPromptInfo resolveAttackPromptInfo() {
        ...
        class_1058 sprite = atlas.method_4608(ATTACK_PROMPT);
        if (sprite == null) {
            return new AttackPromptInfo(32, 64);
        }
        int texW = sprite.method_45851().method_45807();
        int texH = sprite.method_45851().method_45815();
        if (texW <= 0 || texH <= 0) {
            return new AttackPromptInfo(32, 64);
        }
    ...
    private static SpriteSize resolveSpriteSize(class_2960 id, int fallbackW, int fallbackH) {
        class_310 client = class_310.method_1551();
        if (client == null) {
    ...
    private static float resolveAttackProgressStep() {
        ...
        float adjustedHealth = class_3532.method_16439((float)defeatedEase, (float)healthFactor, (float)1.0f);
        float adjustedEnergy = class_3532.method_16439((float)defeatedEase, (float)energyFactor, (float)1.0f);
        float adjustedRng = class_3532.method_16439((float)defeatedEase, (float)rngScale, (float)1.0f);
        float multiplier = class_3532.method_16439((float)defeatedEase, (float)1.0f, (float)4.0f);
    ...
    private static float resolveDefeatedEaseProgress() {
        ...
        Long startTick = AfwClientAnimationRuntime.findStartTickForActor((UUID)client.field_1724.method_5667());
        if (startTick == null) {
            return 0.0f;
        }
        long elapsed = Math.max(0L, client.field_1687.method_75260() - startTick);
        return class_3532.method_15363((float)((float)elapsed / 300.0f), (float)0.0f, (float)1.0f);
    }
    ...
    private static float resolveAttackHealthFactor() {
        ...
        rawFactor = class_3532.method_15363((float)rawFactor, (float)0.0f, (float)1.0f);
        ...
        return class_3532.method_15363((float)factor, (float)0.3f, (float)1.0f);
    }

    private static float resolveAttackEnergyFactor() {
        class_746 class_7462;
        class_310 client = class_310.method_1551();
        if (client == null || !((class_7462 = client.field_1724) instanceof EnergyHolder)) {
            return 1.0f;
        }
